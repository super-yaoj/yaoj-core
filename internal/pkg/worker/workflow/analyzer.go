...
func (r DefaultAnalyzer) Analyze(w *RtWorkflow) Result {
	...
	for name, node := range w.RtNodes {
		...
		if node.Result.Code != processor.Ok {
			...
			res.Score = 0
			res.File = append(res.File, ResultFile{
				Title:   "message",
...
// TODO: 优化草率的实现
func autoFileDisplay(node *RtNode) []ResultFile {
	switch node.ProcName {
	case "checker:testlib":
		return []ResultFile{FileDisplay(node.Input["answer"], "answer", 5000)}
	case "compiler:testlib", "compiler:auto":
		return []ResultFile{FileDisplay(node.Output["log"], "compile log", 5000)}
	case "runner:auto":
		return []ResultFile{
			FileDisplay(node.Output["stdout"], "stdout", 5000),
			FileDisplay(node.Output["stderr"], "stderr", 5000),
		}
	default:
		return nil
	}
...
func FileDisplay(store data.FileStore, title string, length int) ResultFile {
	bytes, _ := store.Get()
